...
function EncodingParametersInURLs() { // Declaration of the EncodingParametersInURLs functional component.
  ...
  const fetchWelcome = async () => {
    const response = await axios.get("http://kanbas-node-server-app-gucen.onrender.com/a5/welcome"); // Makes an HTTP GET request to the specified URL.
  ...
  const fetchSum = async (a, b) => {
    const response = await axios.get(`http://kanbas-node-server-app-gucen.onrender.com/a5/add/${a}/${b}`); // Makes an HTTP GET request to add 'a' and 'b'.
  ...
  const fetchSubtraction = async (a, b) => {
    const response = await axios.get(`http://kanbas-node-server-app-gucen.onrender.com/a5/subtract/${a}/${b}`); // Makes an HTTP GET request to subtract 'b' from 'a'.
    setResult(response.data); // Updates the 'result' state variable with the response data.
  };
  ...
  return (
    <div>
      ...
      <a href={`http://kanbas-node-server-app-gucen.onrender.com/a5/add/${a}/${b}`} className="btn btn-primary">Add {a} + {b}</a>
      <a href={`http://kanbas-node-server-app-gucen.onrender.com/a5/subtract/${a}/${b}`} className="btn btn-danger">Subtract {a} - {b}</a>

      <h3>Query Parameters</h3> 
      {/* // Anchor tags with href attributes dynamically created to show how query parameters can be used in URLs. */}
      <a href={`http://kanbas-node-server-app-gucen.onrender.com/a5/calculator?operation=add&a=${a}&b=${b}`} className="btn btn-primary">Add {a} + {b}</a>
      <a href={`http://kanbas-node-server-app-gucen.onrender.com/a5/calculator?operation=subtract&a=${a}&b=${b}`} className="btn btn-danger">Subtract {a} - {b}</a>
    </div>
  );
}
